...
class RewardedDefault : NSObject, LPMRewardedAdDelegate, Rewarded {
    ...
    func Init(ui: RewardedUi) {
        ...
        _rewarded = LPMRewardedAd(adUnitId: RewardedUi.AdUnitA)
    ...
    public func Load() {
        ISNeftaCustomAdapter.onExternalMediationRequest(withRewarded: _rewarded, adUnitId: RewardedUi.AdUnitA, insight: nil)
        _rewarded.loadAd()
    }
    
    public func Show() {
        if _rewarded.isAdReady() {
            _rewarded.showAd(viewController: _ui.ViewController, placementName: nil)
